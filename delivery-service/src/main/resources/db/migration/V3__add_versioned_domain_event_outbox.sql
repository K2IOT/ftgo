-- Phase 03 Task 1: optimistic Delivery version and stable outbox identity.
...
SET event_id = LOWER(CONCAT(
    SUBSTRING(MD5(CONCAT('delivery-outbox-', id)), 1, 8), '-',
    SUBSTRING(MD5(CONCAT('delivery-outbox-', id)), 9, 4), '-',
    SUBSTRING(MD5(CONCAT('delivery-outbox-', id)), 13, 4), '-',
    SUBSTRING(MD5(CONCAT('delivery-outbox-', id)), 17, 4), '-',
    SUBSTRING(MD5(CONCAT('delivery-outbox-', id)), 21, 12)
))
