 ...
 * - Requirement 2.3: Settle payment using void or refund from durable state
...
class CancelOrderSagaTest {
    ...
    void testSagaHasCorrectStructure() {
        ...
        // 3. settlePayment (PIVOT POINT - no compensation)
        // 4. recordFinancialSettlement (local)
        // 5. confirmCancelTicket (retriable)
        // 6. confirmCancel (local participant, retriable)
    }
    ...
    @Test
    void cancellationAcceptedByKitchenVoidsAuthorizationAndCompletesCancellation() {
        ...
        SagaUnitTestSupport.given()
            ...
            .successReply()
            .expect()
            .command(new VoidAuthorizationCommand(
                1L,
                123L,
                "ORDER_CANCELLED",
                "cancel-order-1-authorization-123-void"
            ))
        ...
        verify(localSteps).recordFinancialSettlement(
            1L,
    ...
    @Test
    void kitchenRejectionRestoresOrderWithoutSettlingPayment() {
        CancelOrderSagaLocalSteps localSteps = mock(CancelOrderSagaLocalSteps.class);
        CancelOrderSaga saga = new CancelOrderSaga(localSteps);
        ...
        verify(localSteps, never()).recordFinancialSettlement(anyLong(), any(), anyLong(), any());
    }
}
