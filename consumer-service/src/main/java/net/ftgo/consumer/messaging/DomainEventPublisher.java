...
public class DomainEventPublisher {
    ...
    public void publishConsumerEvent(Long aggregateId, Object event) {
        publishConsumerEvent(aggregateId, 0L, DomainEventMetadata.empty(), event);
    }

    @Transactional
    public void publishConsumerEvent(Long aggregateId, long aggregateVersion, Object event) {
        publishConsumerEvent(aggregateId, aggregateVersion, DomainEventMetadata.empty(), event);
    }

    @Transactional
    public void publishConsumerEvent(
        Long aggregateId,
        ...
        try {
            ...
            logger.info(
                "Published eventType={} eventId={} for Consumer {} at version {}",
                eventType,
                envelope.eventId(),
                aggregateId,
                aggregateVersion
            ...
            logger.error("Failed to serialize eventType={} for Consumer {}", eventType, aggregateId, e);
            throw new RuntimeException("Failed to publish event", e);
        } catch (RuntimeException e) {
            outboxMetrics.recordPublishError();
            logger.error("Failed to insert eventType={} for Consumer {}", eventType, aggregateId, e);
            throw e;
        }
    }
}
