...
class SecuritySmokeTest {
    ...
    @Test
    void rejectsDirectOrderHistoryRequestWithoutToken() throws Exception {
    ...
    void rejectsAuthenticatedUserWithoutConsumerRole() throws Exception {
        mockMvc.perform(get("/api/security-probe").with(jwt().authorities(
    ...
    @Test
    void permitsConsumerTokenForOrderHistoryEndpoint() throws Exception {
        mockMvc.perform(get("/api/security-probe").with(jwt().authorities(
                new SimpleGrantedAuthority("ROLE_CONSUMER"),
                new SimpleGrantedAuthority("AUD_ftgo-api")
            )))
